...
class cluedoMain {
    ...
    public void startPlayingGame() {
        ...
        while (!players.isEmpty()){
            Player player1 = players.get(count); // get the current player

        }
                gui.createMainFrame(player1);//

        if (gui.passNextPlayer){
            count+=1;
            gui.createMainFrame(players.get(count));
...
//
//                Player player1 = players.get(i); // get the current player
//                System.out.println("PlayERS TURN: " + player1.getName());
//                gui.createMainFrame(player1); //
//
